 ...
 * Parse "Load list"
 */
public final class LoadlistParser extends SingleSheetParser {
    ...
    public void parse() {
        ...
        if (firstRow == null) {
            throw new RuntimeException("Error in sheet '" + sheet.getSheetName() + "' no rows found, sheet is empty");
        }
        final String sheetTypeIndicator = cellString(firstRow.getCell(0)); // 0 CONTAINER_ID or CONTAINER_COUNT
        if (sheetTypeIndicator == null || sheetTypeIndicator.length() == 0) {
            throw new RuntimeException("Error in sheet '" + sheet.getSheetName()
        ...
        if (sheetTypeIndicator.equals("CONTAINER_ID")) {
            ...
            throw new RuntimeException("Error in sheet '" + sheet.getSheetName()
                    + "' first cell value is unknown, expected value 'CONTAINER_ID' or 'CONTAINER_COUNT'," + " got '"
                    + sheetTypeIndicator + "'");
        }
        ...
        for (int r = 1; r < sheet.getPhysicalNumberOfRows(); ++r) { // Skip first row, content ignored
            ...
            if (row == null) { // check for inconsistency,
                // seen in Excel sheets generated from LibreOffice 4.0.4.2 where deleting rows did not set consistent
            ...
            try {
                ...
                if (this.isProjections) { // projections - expand container list
                    ...
                    if (containerNumberAsString == null || containerNumberAsString.length() == 0) {
                        addSheetWarning("row " + row.getRowNum() + " ignoring loadlist row without CONTAINER_COUNT");
                    ...
                    if (containerCount == null || containerCount < 1 || containerCount > 9999) {
                        throw new RuntimeException(
                                "Projections: expected a number 1 <= n <= 9999 in field CONTAINER_COUNT, got '"
                                        + containerCount + "'");
                    }
                } else { // plain load list
                    containerId = cellString(row.getCell(0)); // A 0 CONTAINER_ID
                    if (containerId == null || containerId.length() == 0) {
                        addSheetWarning("row " + row.getRowNum() + " ignoring loadlist row without CONTAINER_ID");
                        continue; // Skip containers without container ids
                    }
                    containerCount = 1;
                }

                int containerIdCounter = this.randomGenerator.nextInt(9990000); // allows for additional 9999 generated
                                                                                // containers
                ...
                for (int i = 0; i < containerCount; ++i) {
                    if (this.isProjections) {
                        ...
                        if (this.stdDevContainerWeight > 0 && containerCount > 1) {
                            /**
                             * Generating containers with a fixed standard deviation on the weight since real world
                             * cargo distributions have a weight spread
                             */
                            long newWeight = builder.getContainerWeight()
                                    + Math.round(this.stdDevContainerWeight * this.randomGenerator.nextGaussian());
                ...
                if (ediFactType == EdiFactType.COPRAR && !loadPort.equals(rowLoadPort)) {
                    addSheetWarning("row " + row.getRowNum()
                            + ": COPRAR containers must have same load port, expected '" + loadPort + "', got '"
                            + rowLoadPort + "'");
                }
                if (voyageCode == null) {
                ...
                addSheetWarning("row " + row.getRowNum() + " error: " + e.getMessage());
                throw new RuntimeException("Error in sheet '" + sheet.getSheetName() + "' row " + row.getRowNum(), e);
            }
        }
